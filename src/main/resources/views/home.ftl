...
<div class="header z-50  p-3 justify-evenly font-poppins font-bold flex items-center text-left w-screen bg-gray-700 fixed top-0 text-black">
  ...
  <div class="form-container mt-12 font-poppins border shadow-md w-full sm:w-1/2 m-auto p-4 mb-4 rounded-md" id="${alert.tabId}">
        ...
        try {
            const response = await axios.post("http://localhost:8080/api/transaction/confirm",
                ...
                if(response.status===200){
                    window.location.href="/success/"+mobileNumber;
                }else if(response.status===201){
                     window.location.href="/canceled/"+mobileNumber;
                }else{
                    alert("server error!");
                }
        }catch(err){
